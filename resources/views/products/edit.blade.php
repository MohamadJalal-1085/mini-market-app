...
<body class="bg-[var(--bg)] text-[var(--text)] font-['Inter'] antialiased">
    ...
    <div class="container mx-auto p-4 sm:p-6 lg:p-8 max-w-4xl">
        ...
        <header class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-semibold" style="font-size: clamp(22px, 3.4vw, 32px);">Edit Product</h1>
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full bg-[#0b131c] border border-[var(--border)] rounded-xl px-4 py-2.5 focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--ring)]/60 focus:outline-none transition-all">{{ old('description', $product->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="quantity" name="quantity" required value="{{ old('quantity', $product->quantity) }}"
                               class="w-full bg-[#0b131c] border border-[var(--border)] rounded-xl px-4 py-2.5 focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--ring)]/60 focus:outline-none transition-all">
                    </div>
                </div>
                ...
                {{-- Image Upload --}}
                <div class="mb-8">
                    <label for="image" class="block text-sm font-medium text-[var(--muted)] mb-2">Update Product Image</label>
                    @if($product->image)
                        <div class="flex items-center gap-4 mb-4">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="Current Image" class="w-16 h-16 rounded-lg object-cover">
                            <span class="text-sm text-[var(--muted)]">Current image</span>
                        </div>
                    @endif
